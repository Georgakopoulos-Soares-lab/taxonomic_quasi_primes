...
int main(int argc, char* argv[]) {
    ...
    string line;
    int fileCounter = 0;
    unordered_map<string, int> superkingdomFileCounts;

    while (getline(mappingsFile, line)) {
        size_t spacePos = line.find_first_of(" ");
        string superkingdom = line.substr(spacePos + 1);
        superkingdomFileCounts[superkingdom]++;
    }
    cout << "Total number of superkingdoms are: " << superkingdomFileCounts.size() << endl;
    mappingsFile.close();
    ...
    while (getline(mappingsFile, line)) {
        ...
        while (file >> kmer) {
            ...
            if (it != kmerMap.end()) {
                if (it->second.first != superkingdom) it->second.first = "cross-superkingdom";
                else ++it->second.second;
            } else {
                kmerMap[kmer] = {superkingdom, 1};
            }
        }
        file.close();
    ...
    for (const auto& kv : kmerMap) {
        if (kv.second.first != "cross-superkingdom") {
            float percentage = static_cast<float>(kv.second.second) /
                               static_cast<float>(superkingdomFileCounts[kv.second.first]) * 100.0f;
